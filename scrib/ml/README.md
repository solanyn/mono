...
ML audio pipeline for [scrib](../) meeting recordings. Runs on Apple Silicon (macOS only).
...
bazel run //scrib/ml:create_venv
source scrib/ml/venv/bin/activate
python -m scrib_ml.server --host 127.0.0.1 --port 8002 --workers 1
```

### Update dependencies

```bash
bazel run //scrib/ml:generate_requirements_txt
...
```bash
bazel test //scrib/ml:test_align
```

### Build

```bash
bazel build //scrib/ml:scrib-ml
```

## Deployment

Runs as a launchd agent on Mac Mini (`com.scrib.ml`), managed by nix-darwin.

```nix
# In nix-darwin hosts/personal.nix
launchd.user.agents.scrib-ml = {
  command = "/opt/homebrew/bin/uv run --project /Users/andrew/git/mono/scrib/ml python -m scrib_ml.server --host 0.0.0.0 --port 8002 --workers 1";
  ...
};
```
