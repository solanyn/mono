...
mod tests {
    ...
    use salsa20::cipher::{KeyIvInit, StreamCipher, generic_array::GenericArray}; // For test utility
    ...
    fn test_parse_invalid_magic_value_with_sample_structure() {
        let decrypted_payload = match crate::cypher::decrypt(&SAMPLE_ENCRYPTED_PACKET) {
            Ok(payload) => payload,
            Err(e) => panic!("Failed to decrypt sample packet for magic value corruption test: {:?}", e),
        };
        ...
        let original_iv_seed_from_sample: [u8; crate::cypher::IV_SIZE] = 
            SAMPLE_ENCRYPTED_PACKET[crate::cypher::IV_OFFSET..(crate::cypher::IV_OFFSET + crate::cypher::IV_SIZE)]
            .try_into().unwrap();
        ...
        final_encrypted_corrupted_packet[crate::cypher::IV_OFFSET..(crate::cypher::IV_OFFSET + crate::cypher::IV_SIZE)]
            .copy_from_slice(&SAMPLE_ENCRYPTED_PACKET[crate::cypher::IV_OFFSET..(crate::cypher::IV_OFFSET + crate::cypher::IV_SIZE)]);

        match Packet::try_from(&final_encrypted_corrupted_packet) {
            Err(ParsePacketError::InvalidMagicValue(val)) => {
                assert_eq!(val, bad_magic_val, "The magic value reported in the error should be the corrupted one.");
            }
            Ok(p) => panic!("Expected InvalidMagicValue error, but parsing succeeded with packet: {:#?}", p),
            Err(e) => panic!("Expected InvalidMagicValue error, got different error: {:?}", e),
        }
    }
}
