use crate::{
    constants::{PACKET_DECRYPTION_KEY, PACKET_SIZE},
    errors::CypherError,
};
use salsa20::cipher::{KeyIvInit, StreamCipher, generic_array::GenericArray};
use salsa20::Salsa20;

// Public constants for potential use in packet.rs tests or other modules if necessary
pub const EXPECTED_KEY_LEN: usize = 32;
pub const IV_OFFSET: usize = 0x40; // 64
pub const IV_SIZE: usize = 4;
pub const MIN_DATA_LEN_FOR_IV: usize = IV_OFFSET + IV_SIZE; // 68

pub fn decrypt(data: &[u8; PACKET_SIZE]) -> Result<[u8; PACKET_SIZE], CypherError> {
    if PACKET_DECRYPTION_KEY.len() < EXPECTED_KEY_LEN {
        return Err(CypherError::InvalidKeyLength(PACKET_DECRYPTION_KEY.len()));
    }
    // Ensure key is correct length before creating GenericArray
    let key_bytes_slice = &PACKET_DECRYPTION_KEY[0..EXPECTED_KEY_LEN];
    let key_slice = GenericArray::from_slice(key_bytes_slice);

    // This check is technically redundant if PACKET_SIZE is a const >= MIN_DATA_LEN_FOR_IV
    // but good for explicit safety if PACKET_SIZE could vary or be misconfigured.
    if PACKET_SIZE < MIN_DATA_LEN_FOR_IV {
        return Err(CypherError::InputTooShortForIv(PACKET_SIZE));
    }

    let original_init_vector_slice = &data[IV_OFFSET..(IV_OFFSET + IV_SIZE)];
    let original_init_vector: [u8; IV_SIZE] = match original_init_vector_slice.try_into() {
        Ok(arr) => arr,
        Err(_) => return Err(CypherError::InvalidIvStructure), // Should not happen with correct slicing & IV_SIZE
    };

    let iv1 = u32::from_le_bytes(original_init_vector);
    let iv2 = iv1 ^ 0xDEADBEAF; 
    let mut iv_salsa20_bytes: [u8; 8] = [0u8; 8];
    iv_salsa20_bytes[0..4].copy_from_slice(&iv2.to_le_bytes());
    iv_salsa20_bytes[4..].copy_from_slice(&iv1.to_le_bytes());
    let iv_generic_array = GenericArray::from_slice(&iv_salsa20_bytes);

    let mut cipher = Salsa20::new(key_slice, iv_generic_array);
    let mut decrypted_buf = *data;
    
    cipher.apply_keystream(&mut decrypted_buf[..]);

    Ok(decrypted_buf)
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::constants::PACKET_MAGIC_VALUE; // For checking decrypted output

    const SAMPLE_ENCRYPTED_PACKET_FOR_CYPHER_TEST: [u8; PACKET_SIZE] = [
        ,
    ];

    #[test]
    fn test_decrypt_sample_packet_does_not_error() {
        match decrypt(&SAMPLE_ENCRYPTED_PACKET_FOR_CYPHER_TEST) {
            Ok(decrypted_output) => {
                if decrypted_output.len() >= 4 {
                    // .unwrap() is safe here because we just checked length
                    let magic_in_decrypted = u32::from_le_bytes(decrypted_output[0..4].try_into().unwrap());
                    assert_eq!(magic_in_decrypted, PACKET_MAGIC_VALUE, 
                               "Decrypted output should start with the known magic value.");
                    println!("Decryption of sample packet succeeded and magic value matches.");
                } else {
                    panic!("Decrypted output is too short."); // Should not happen given PACKET_SIZE
                }
            }
            Err(e) => panic!("Decryption failed for the sample packet: {:?}", e),
        }
    }
}
