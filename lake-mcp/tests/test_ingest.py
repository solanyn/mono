...
from lake_mcp.ingest.rba_csv import fetch_rba_csv, parse_rba_csv, build_bronze_table
from lake_mcp.ingest.abs_sdmx import (
    fetch_abs_cpi,
    parse_observations as parse_abs,
    build_bronze_table as build_abs_bronze,
)
from lake_mcp.ingest.aemo import (
    fetch_nem_summary,
    build_bronze_table as build_aemo_bronze,
)
from lake_mcp.ingest.reddit import fetch_reddit, build_bronze_table as build_reddit_bronze
from lake_mcp.ingest.rss import fetch_feeds, build_bronze_table as build_rss_bronze
...
class TestRbaIngest:
    ...
    @patch("lake_mcp.ingest.rba_csv.httpx.get")
...
class TestAbsIngest:
    ...
    @patch("lake_mcp.ingest.abs_sdmx.httpx.get")
...
class TestAemoIngest:
    def test_parse_nem_summary(self, aemo_nem_json):
        with patch("lake_mcp.ingest.aemo.httpx.get") as mock_get:
...
class TestRedditIngest:
    def test_parse_posts(self, reddit_hot_json):
        with patch("lake_mcp.ingest.reddit.httpx.get") as mock_get:
    ...
    def test_deduplication(self, reddit_hot_json, reddit_new_json):
        ...
        with patch("lake_mcp.ingest.reddit.httpx.get", side_effect=side_effect):
...
class TestRssIngest:
    def test_parse_feeds(self, rss_guardian_xml):
        with patch("lake_mcp.ingest.rss.httpx.get") as mock_get:
